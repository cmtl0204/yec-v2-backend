...
interface CreateEnrollmentParams {
  ...
  workdayId: string;
  enrollmentStateEnrolledId: string;
  userId: string;
  subjectId: string;
  attendance: number;
  finalGrade: number;
...
export class PlacementTestsService {
  ...
  async importEnrollments(file: Express.Multer.File) {
    ...
    const subjectA11 = subjects.find(subject => subject.code.toLowerCase() === 'yec-a1.1');
    const subjectA12 = subjects.find(subject => subject.code.toLowerCase() === 'yec-a1.2');
    const subjectA21 = subjects.find(subject => subject.code.toLowerCase() === 'yec-a2.1');
    const subjectA22 = subjects.find(subject => subject.code.toLowerCase() === 'yec-a2.2');
    const subjectB11 = subjects.find(subject => subject.code.toLowerCase() === 'yec-b1.1');
    const subjectB12 = subjects.find(subject => subject.code.toLowerCase() === 'yec-b1.2');
      ...
      switch (subject.code) {
        case 'YEC-A1.2': {
          await this.createEnrollment({
            subjectId: subjectA11.id,
            enrollmentStateEnrolledId: enrollmentStateEnrolled.id,
            userId: student.userId,
            enrollmentTypeId: enrollmentType.id,
            academicPeriod: subjectA11.academicPeriod,
            career: career,
            identification,
  ...
  async createEnrollment({
    ...
    if (!enrollment) {
      ...
      enrollment.parallelId = parallelId;
      enrollment.schoolPeriodId = schoolPeriod.id;
      enrollment.studentId = studentId;
      enrollment.typeId = enrollmentTypeId;
      enrollment.workdayId = workdayId;
  ...
  async saveAttendance(attendance: number, enrollmentDetail: EnrollmentDetailEntity) {
    enrollmentDetail.finalAttendance = attendance;

    await this.enrollmentDetailRepository.update(enrollmentDetail.id, enrollmentDetail);
  }

  async saveAcademicState(finalGrade: number, enrollmentDetail: EnrollmentDetailEntity) {
    enrollmentDetail.finalGrade = finalGrade;
