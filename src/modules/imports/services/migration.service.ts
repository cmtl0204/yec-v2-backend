...
export class MigrationService {
    ...
    async import(file: Express.Multer.File, payload: any) {
        ...
        for (const item of dataExcel) {
            ...
            const parallel = parallels.find(parallel => parallel.code === item[ColumnsEnum.PARALLEL].toString());
            const enrollmentType = enrollmentTypes.find(enrollmentType => enrollmentType.code === 'ordinary');
            const workday = workdays.find(workday => workday.name == item[ColumnsEnum.WORKDAY].toString().trim());
            ...
            if (enrollment) {
                ...
                enrollment.code = `${schoolPeriod.code}-${career.code}-${identification}`;
